...
def scrapeShots(gameIDs):
    for gameID in gameIDs:
        ...
        for i, profile in enumerate(homeprofiles, 0):
            ...
            firstname = ""
            firstnamediv = profile.find("div", { "class" : "first-name" } )
            if firstnamediv is not None:
                firstname = firstnamediv.text
            lastname = ""
            lastnamediv = profile.find("div", { "class" : "last-name" } )
            if lastnamediv is not None:
                lastname = lastnamediv.text
